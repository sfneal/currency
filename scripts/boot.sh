...
echo "Building image: stephenneal/currency:${TAG}${COMPOSER_FLAGS:8}"
docker build -t stephenneal/currency:"${TAG}${COMPOSER_FLAGS:8}" \
...
docker logs -f currency

while true; do
    if [[ $(docker inspect -f '{{.State.Running}}' currency) != true ]]; then
        break
    else
...
docker inspect -f '{{.State.ExitCode}}' currency > /dev/null 2>&1

# Confirm the image exists
docker image inspect stephenneal/currency:"${TAG}" > /dev/null 2>&1
